...
const inputClass =
  'w-full rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-900 px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 outline-none transition focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-500/30';
...
function ProfileHeader({ user }: { user: User }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
      <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-100 dark:bg-indigo-500/15 text-2xl font-semibold text-indigo-700 dark:text-indigo-300">
        {initial}
      </div>
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {tiles.map((t) => (
        ...
        >
          ...
          <div className="mt-1.5 text-2xl font-semibold tracking-tight text-gray-900 dark:text-gray-50 tabular-nums">
            {t.value}
          </div>
        </div>
      ))}
...
function DetailsCard({ user, onSaved }: { user: User; onSaved: (u: User) => void }) {
  ...
  return (
    ...
    >
      <h2 className="mb-4 text-sm font-semibold text-gray-900 dark:text-gray-50">Details</h2>
      ...
      <button
        type="submit"
        disabled={busy || !dirty}
        className="mt-4 rounded-xl bg-indigo-600 dark:bg-indigo-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-indigo-700 dark:hover:bg-indigo-600 disabled:cursor-not-allowed disabled:opacity-50"
      >
...
function PasswordCard() {
  ...
  return (
    <form
      ...
      className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5"
    >
      <h2 className="mb-4 text-sm font-semibold text-gray-900 dark:text-gray-50">Password</h2>
      <div className="space-y-3">
        <input
          type="password"
          value={current}
