...
export function PricingSettings() {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6">
      <h2 className="mb-1 text-base font-semibold text-gray-900">Model pricing</h2>
            ...
            <tbody>
              {prices.map((p) => (
                <tr key={p.model} className="border-b border-gray-50 last:border-0">
                  <td className="py-2.5 text-gray-800">{p.model}</td>
                  ...
                  <td className="py-2.5 text-right">
                    <button
                      onClick={() => edit(p)}
                      className="rounded-lg px-2 py-1 text-xs font-medium text-gray-600 transition hover:bg-gray-100"
                    >
                      Edit
                    </button>
                    {p.source === 'custom' && (
                      <button
                        ...
                        disabled={busy}
                        className="rounded-lg px-2 py-1 text-xs font-medium text-red-600 transition hover:bg-red-50 disabled:opacity-50"
                      >
                        Reset
                      </button>
                    )}
      ...
      <form onSubmit={save} className="mt-5 border-t border-gray-100 pt-4">
        ...
        <div className="mt-2 flex flex-wrap items-end gap-2">
          ...
          <div className="w-32">
            <label className="mb-1 block text-xs text-gray-500">Input $/1M</label>
            <input
              type="number"
              min={0}
              step="0.01"
          ...
          <div className="w-32">
            <label className="mb-1 block text-xs text-gray-500">Output $/1M</label>
            <input
              type="number"
              min={0}
              step="0.01"
