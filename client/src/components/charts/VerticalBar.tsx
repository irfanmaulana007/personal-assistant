...
function VBTooltip({ active, payload }: { active?: boolean; payload?: TooltipEntry[] }) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-xs shadow-sm">
      <div className="font-medium text-gray-900">{p.value.toLocaleString()}</div>
      <div className="text-gray-400">{p.label}</div>
    </div>
  );
}

export function VerticalBar({ data, color = '#4f46e5' }: { data: Datum[]; color?: string }) {
    ...
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
        ...
        <YAxis
          ...
          allowDecimals={false}
        />
        <Tooltip content={<VBTooltip />} cursor={{ fill: '#f3f4f6' }} />
