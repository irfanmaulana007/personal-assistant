...
import { useGlobalDashboard } from './util';

// GlobalUsers ranks the most active users across every selected project. A user
// active in several projects is merged into a single row.
export function GlobalUsers() {
  const { aggregate } = useGlobalDashboard();
  const { formatMoney } = usePreferences();
  const users = aggregate.by_user;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatTile label="Total tokens" value={formatTokens(s.total_tokens)} />
        <StatTile label="Est. cost" value={formatMoney(s.estimated_cost_usd)} />
      </div>

      <Card title="Top users (all projects)">
        {users.length === 0 ? (
