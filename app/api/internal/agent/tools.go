...
var skillTools = map[string][]toolSpec{
	...
	"trello_review": {
		{
			name:        "trello_review",
			description: "List every task and bug on this project's Trello board, grouped by list (e.g. Backlog / Todo / In Progress / Done / Bug). Use when the user wants to see, check, review, or summarize their tasks, backlog, or open bugs (e.g. 'what's on my board', 'list my tasks', 'any open bugs', 'review the backlog').",
	...
	"trello_card": {
		...
		},
		{
			...
			description: "File a NEW bug report on this project's Trello board (its Bug list). Use this when the user reports that something already built behaves wrong — broken, erroring, crashing, wrong result, or not doing what it should. Enrich the fields from what the user said and write them in English.",
			capability:  intent.CapabilityTrello,
		...
		},
		{
			...
			description: "Edit an EXISTING task card on this project's Trello board — retitle it, rewrite its description, replace its acceptance criteria, change or remove its type label, and/or move it to another list (e.g. Backlog / Todo / In Progress / Done, to mark a task in progress or done). Identify the card by its current title (or a distinctive part of it, as shown by trello_review). Only pass the fields you want to change; omitted fields are left untouched. Use this when the user wants to change, edit, rename, re-scope, relabel, or move a task that already exists — NOT to create a new one (use trello_create_task). Write any text in English.",
			capability:  intent.CapabilityTrello,
