...
var skillTools = map[string][]toolSpec{
	...
	"trello_review": {
		{
			name:        "trello_review",
			description: "List every task and bug across ALL of this project's linked Trello boards, grouped by board and then by list (e.g. Backlog / Todo / In Progress / Done / Bug). Use when the user wants to see, check, review, or summarize their tasks, backlog, or open bugs (e.g. 'what's on my board', 'list my tasks', 'any open bugs', 'review the backlog'). The output's board headings are the board names to pass to the other Trello tools.",
	...
	"trello_card": {
		{
			...
			description: "File a NEW work task on one of this project's linked Trello boards (its Backlog/Todo list). Use this for project/development work items for the app — a new feature, an improvement, a chore, or a refactor — NOT for personal life goals (those belong on the bucket list, via bucketlist_add). Enrich the fields from what the user said and write them in English. Decide the label from the nature of the work: feature (new capability), improvement (enhance something that exists), chore (maintenance/config/docs, no behaviour change), refactor (restructure without behaviour change).",
			capability:  intent.CapabilityTrello,
			action:      intent.ActionTrelloCreateTask,
			parameters:  `{"type":"object","properties":{"title":{"type":"string","description":"Short imperative summary of the work, in English (e.g. 'Add dark-mode toggle to settings')."},"description":{"type":"string","description":"1-3 sentences of context in English: what needs doing and why."},"acceptance_criteria":{"type":"string","description":"The concrete, verifiable conditions for the task to be done, in English — 2-5 items, ONE PER LINE (newline-separated), each a testable statement. Do not number them."},"label":{"type":"string","enum":["feature","improvement","chore","refactor"],"description":"The task type, choosing exactly one."},"board":{"type":"string","description":"Which linked Trello board to file this on, by name (as shown by trello_review's board headings). Omit if the project has only one board. If several are linked and the user didn't say which, ask them first — don't guess."}},"required":["title","description","acceptance_criteria","label"]}`,
		},
		{
			name:        "trello_report_bug",
			description: "File a NEW bug report on one of this project's linked Trello boards (its Bug list). Use this when the user reports that something already built behaves wrong — broken, erroring, crashing, wrong result, or not doing what it should. Enrich the fields from what the user said and write them in English.",
			capability:  intent.CapabilityTrello,
			action:      intent.ActionTrelloReportBug,
			parameters:  `{"type":"object","properties":{"title":{"type":"string","description":"Short summary of the defect, in English (e.g. 'Login button unresponsive on mobile Safari')."},"description":{"type":"string","description":"Context and, when known, the steps to reproduce, in English."},"actual_result":{"type":"string","description":"What currently happens — the wrong behaviour — in English."},"expected_result":{"type":"string","description":"What should happen instead, in English."},"board":{"type":"string","description":"Which linked Trello board to file this on, by name (as shown by trello_review's board headings). Omit if the project has only one board. If several are linked and the user didn't say which, ask them first — don't guess."}},"required":["title","actual_result","expected_result"]}`,
		},
		{
			name:        "trello_update_card",
			description: "Edit an EXISTING task card on one of this project's linked Trello boards — retitle it, rewrite its description, replace its acceptance criteria, change or remove its type label, and/or move it to another list (e.g. Backlog / Todo / In Progress / Done, to mark a task in progress or done). Identify the card by its current title (or a distinctive part of it, as shown by trello_review). Only pass the fields you want to change; omitted fields are left untouched. Use this when the user wants to change, edit, rename, re-scope, relabel, or move a task that already exists — NOT to create a new one (use trello_create_task). Write any text in English.",
			capability:  intent.CapabilityTrello,
			action:      intent.ActionTrelloUpdateCard,
			parameters:  `{"type":"object","properties":{"card":{"type":"string","description":"Which task to update: its current title, or a distinctive part of it, exactly as it appears on the board (from trello_review)."},"title":{"type":"string","description":"New title for the card. Omit to keep the current title."},"description":{"type":"string","description":"New description/context for the card, in English. Replaces the current description. Do NOT put acceptance criteria here — use acceptance_criteria. Omit to keep the current description."},"acceptance_criteria":{"type":"string","description":"Replacement acceptance criteria — 2-5 testable items, ONE PER LINE (newline-separated), no numbering. Replaces both the Acceptance Criteria section and the trackable checklist. Omit to leave them unchanged."},"label":{"type":"string","enum":["feature","improvement","chore","refactor","none"],"description":"Change the task type label; use 'none' to remove all labels. Omit to leave the label unchanged."},"list":{"type":"string","description":"Move the card to the list with this name on the board (e.g. Backlog, Todo, In Progress, Done — as shown by trello_review). Omit to leave it where it is."},"board":{"type":"string","description":"Which linked Trello board the card is on, by name (as shown by trello_review's board headings). Omit if the project has only one board. If several are linked and the user didn't say which, ask them first — don't guess."}},"required":["card"]}`,
		},
		{
			name:        "trello_archive_card",
			description: "Archive (close) an EXISTING card on one of this project's linked Trello boards, removing it from the board and from all future trello_review results. Use when the user wants to archive, close, clear, or remove a task, bug, or card that already exists (e.g. 'archive the dark-mode task', 'remove that bug from the board', 'clean up the done cards'). This does NOT permanently delete data — the card is hidden and can be restored from Trello. Do NOT use it to create a card (trello_create_task) or to merely edit/move one (trello_update_card). Identify the card by its current title (as shown by trello_review) or by its Trello card id. If unsure which card is meant, call trello_review first to get the exact title.",
			capability:  intent.CapabilityTrello,
			action:      intent.ActionTrelloArchiveCard,
			parameters:  `{"type":"object","properties":{"card":{"type":"string","description":"Which card to archive: its current title, a distinctive part of it (exactly as shown by trello_review), or its Trello card id / shortLink."},"board":{"type":"string","description":"Which linked Trello board the card is on, by name (as shown by trello_review's board headings). Omit if the project has only one board. If several are linked and the user didn't say which, ask them first — don't guess."}},"required":["card"]}`,
		},
	},
	"game_idea": {
		{
			name:        "trello_save_game_idea",
			description: "Save a video-game idea as a card on one of this project's linked Trello boards (its Ideas list). Use this when the user shares a concept for a game they might build as a personal/side project — a mechanic, theme, pitch, or a vague one-liner. Do NOT use it for app work items (use trello_create_task / trello_report_bug) or personal life goals (use bucketlist_add). Enrich the idea from what the user said plus your own game-design knowledge, and ALWAYS write the card in English.",
			capability:  intent.CapabilityTrello,
			action:      intent.ActionTrelloGameIdea,
			parameters:  `{"type":"object","properties":{"title":{"type":"string","description":"Short, catchy name for the game idea, in English (e.g. 'Time-rewind puzzle platformer')."},"concept":{"type":"string","description":"A 1-3 sentence pitch of the core idea and what makes it fun or unique, in English."},"genre":{"type":"string","description":"Genre and style, in English (e.g. 'Roguelike deck-builder', '2D metroidvania', 'Cozy farming sim')."},"core_mechanics":{"type":"string","description":"The main gameplay mechanics or core loop, in English — 2-6 items, ONE PER LINE (newline-separated). Do not number them."},"references":{"type":"string","description":"Reference games or inspirations that capture the vibe or mechanics, in English — 2-5 items, ONE PER LINE. Name the game and add a short note or URL where helpful. Fill this in from your own knowledge even if the user named none."},"notes":{"type":"string","description":"Optional extra context in English: target platform, art style, scope, monetization, or open questions."},"board":{"type":"string","description":"Which linked Trello board to save this on, by name (as shown by trello_review's board headings). Omit if the project has only one board. If several are linked and the user didn't say which, ask them first — don't guess."}},"required":["title","concept"]}`,
		},
	},
	"self_tuning": {
		{
