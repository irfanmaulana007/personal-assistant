...
var skillSeed = []Skill{
	...
	},
	{
		...
		Prompt:         "You keep a detailed log of the user's hiking trips. Use hike_log to record a hike, capturing: the mountain/destination, the trail used going up, the trail used going down, whether they camped (yes/no), how many days and how many nights, the hiking date, and the participants (as a comma-separated list). If the trip spans more than a single day (days > 1 or nights > 0), assume they camped — do not ask whether they camped, as it is inferred automatically. Only ask about camping for single-day hikes. Use hike_summary to review past hikes; each hike is shown with a #number (its id). Use hike_update to fix an already-logged hike — most often a wrong date, but also the mountain, trails, or duration — passing that #number as the id and only the fields that change; everything else on the hike (including participants) is preserved, so prefer this over deleting and re-logging. When the user gives dates for several existing hikes at once, call hike_update once per hike. Use hike_delete to remove a hike the user no longer wants — for example a duplicate — passing that #number as the id (you can pass several comma-separated numbers to delete duplicates in one call); run hike_summary first so you delete the right entries. The system automatically matches similar existing mountain, trail, and participant names to prevent duplicates from typos, so pass names as the user says them and mention when it reused an existing name. The mountain is required — ask for it if missing. The date is optional: if the user doesn't give one, log the hike without a date (do NOT invent today's date or ask for it), and they can add it later with hike_update. When the user provides dates for hikes they logged earlier, use hike_update to fill them in.",
	},
	{
		Key:            "web_search",
		Name:           "Web Search",
	...
	},
	{
		...
		DefaultEnabled: false,
		SortOrder:      13,
		Description:    "Turn a chat message into a Trello card — and edit or archive existing ones. Describe a task or a bug and the assistant files it on the right board (a task with acceptance criteria and a type label on the Task Management backlog, or a bug with actual vs. expected result on the Issue board). It can also update an existing task: retitle it, rewrite its description or acceptance criteria, change its label, or move it between lists (e.g. mark it in progress or done). And it can archive a card to clear it off the board when it's done or no longer needed. Cards are always written in English. Requires a Trello API key and token (set them on the Integrations page).",
		Prompt:         "You can file, edit, and archive Trello cards for the user's software project. Decide when to use these tools by the nature of what the user says — be smart about it:\n- If the user wants NEW work on the app (a feature, improvement, chore, or refactor), call trello_create_task to add it to the Task Management → Backlog list. Provide a title, a short description, acceptance criteria (2-5 testable items, one per line), and pick the label (feature / improvement / chore / refactor) from the nature of the work.\n- If the user reports that something already built is BROKEN or behaves wrong, call trello_report_bug to file it on the Issue → Bug list, with a title, description, the actual (wrong) result, and the expected result.\n- If the user wants to CHANGE an existing task — rename it, rewrite its description or acceptance criteria, change or remove its label, or move it to another list (e.g. \"mark the dark-mode task in progress\", \"move X to done\", \"update the acceptance criteria on Y\") — call trello_update_card. Identify the card by its current title, and pass only the fields that change. If you're unsure which card they mean, call trello_review first to see the exact titles.\n- If the user wants to ARCHIVE, close, clear, or remove an existing card from the board (e.g. \"archive the dark-mode task\", \"remove that bug from the board\", \"clear the done cards\"), call trello_archive_card. Identify the card by its current title (or its card id). This hides the card and drops it from future reviews — it is not a permanent delete, and the card can be restored from Trello. If you're unsure which card they mean, call trello_review first to see the exact titles.\n- Do NOT use these for personal life goals or aspirations (\"visit Japan\", \"learn to swim\") — those belong on the bucket list (bucketlist_add), not Trello. Trello is only for project/development work items and bug reports.\nAlways enrich the fields into clear, well-formed content and ALWAYS write the card in English, even if the user wrote in another language. Only claim you created, updated, or archived a card after the tool call returns successfully, and share the card link it returns. If the request is genuinely ambiguous between a task and a bug, ask one short question first. If Trello isn't configured, say so plainly.",
	},
	{
		Key:            "game_idea",
