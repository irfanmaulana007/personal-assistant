...
// (today & tomorrow) at the configured digest time, at most once per day.
func (h *Handler) maybeSendDigest(ctx context.Context, reminders []store.Reminder, now time.Time) {
	if h.settings == nil {
		return
	}
	digest := h.settings.ReminderDigestTime(ctx)
	if digest == "" {
		return
	}
	hh, mm, ok := parseHM(digest)
	if !ok {
		return
	}
	slot := time.Date(now.Year(), now.Month(), now.Day(), hh, mm, 0, 0, h.timezone)
	if now.Before(slot) {
		return // not yet time today
	}
	today := now.Format("2006-01-02")
	if h.settings.ReminderDigestLastSent(ctx) == today {
		return // already sent today
	}
	// Mark today's recap as handled regardless of outcome so it fires at most
	// once; skip sending entirely if we're well past the slot (server was down).
	markSent := func() {
		if err := h.settings.SetReminderDigestLastSent(ctx, today); err != nil {
			h.log.Error("failed to record digest date", "error", err)
		}
	}
	...
	if h.sendFunc != nil {
		if err := h.sendFunc(ctx, h.ownerJID, msg); err != nil {
			h.log.Error("failed to send reminder digest", "error", err)
			return // leave unmarked so it can retry on the next tick within grace
		}
	}
	markSent()
	h.log.Info("reminder digest sent")
}
...
func buildDigest(reminders []store.Reminder, now time.Time, tz *time.Location) string {
	...
	for _, r := range reminders {
		switch {
		case r.RepeatMode == "specific":
			if event, err := time.ParseInLocation("2006-01-02T15:04", r.EventAt, tz); err == nil && inWindow(event) {
				items = append(items, digestItem{event, reminderBody(r)})
			}
		case len(r.Times) == 0:
			if !r.Notified {
				if w := r.RemindAt.In(tz); inWindow(w) {
					items = append(items, digestItem{w, reminderBody(r)})
				}
			}
		default:
			...
			for _, day := range []time.Time{today, today.AddDate(0, 0, 1)} {
				...
				for _, hm := range r.Times {
					...
					slot := time.Date(day.Year(), day.Month(), day.Day(), hh, mm, 0, 0, tz)
					if inWindow(slot) {
						items = append(items, digestItem{slot, reminderBody(r)})
					}
				}
			}
		}
...
// digestWhen renders an occurrence as "Today 3:04 PM" / "Tomorrow 9:00 AM" / a
// dated form for anything further out.
func digestWhen(when, now time.Time, tz *time.Location) string {
	today := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, tz)
	day := time.Date(when.Year(), when.Month(), when.Day(), 0, 0, 0, 0, tz)
	t := when.Format("3:04 PM")
	switch {
	...
	case day.Equal(today.AddDate(0, 0, 1)):
		return "Tomorrow " + t
	default:
		return when.Format("Mon Jan 2 ") + t
	}
}
