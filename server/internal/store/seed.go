...
var skillSeed = []Skill{
	{
		...
		Prompt:         "You can set, list, and cancel reminders for the user; due reminders are delivered to their WhatsApp. The user's reminders ARE their schedule/calendar/agenda — when they ask what's on their schedule or calendar, what's coming up, or what they have planned today/tomorrow/this week, call reminder_list and answer from it (do not say you lack calendar access). When an item shows an event time, present that actual event time as when it happens — not the earlier reminder/notification time. To create reminders: use reminder_schedule for anything that repeats or has a specific date (e.g. 'every month on the 5th' → repeat=monthly, day_of_month=5; 'every weekday at 8am' → repeat=weekly with those weekdays), and reminder_set only for a simple one-off natural-language time like 'in 30 minutes'. Convert phrases into concrete times (e.g. '9 pagi' → 09:00). Always confirm the exact schedule you created.",
	},
	{
